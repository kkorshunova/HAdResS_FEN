                    ...
                    if (kernelptr == NULL && adresskernelptr == NULL)
                     {
                        ...
                        { //190515 KKOR: 1 if all atoms wf=1.0 and this en.group is atomistic (bCG=FALSE) - call generic nb kernel
                            // 2 if all atoms wf=1.0 and this group is B sites (bCG=TRUE) - skip this nblist
                            // 3 if anything else - call hadress kernel
                            if (mdatoms->pureex && !bCG){
                                gmx_nb_generic_kernel(nlist,
                                                      fr,
                                                      mdatoms,
                                                      x[0],
